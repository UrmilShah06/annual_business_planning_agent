# planning_agent.py
# LangGraph 4-node planning workflow: baseline → target cascade → loyalty → word doc
# node 2 uses AutoGen-style proposer/challenger — two GPT-4o calls, one proposes, one challenges
...
class PlanningState(TypedDict):
    ...
    baseline_data: dict          # structured numbers for cascade
    target_plan: str
    target_data: dict            # structured targets
    challenger_feedback: str     # AutoGen challenge output
    loyalty_plan: str
    loyalty_data: dict

    # Human review
    human_adjustments: dict      # {store: adjusted_growth_pct}
    plan_approved: bool
...
def load_assumptions() -> dict:
    """Load all three sheets from planning assumptions."""
...
def to_python_types(obj):
    """Convert numpy types to native Python for LangGraph serialization."""
    import numpy as np
...
def baseline_analysis_node(state):
    ...
    # Build structured baseline data
    baseline_data = {}
...
# --- node 2 — target cascade engine (autogen proposer + challenger) ---

def target_cascade_node(state):
    # AutoGen pattern: proposer sets targets, challenger stress-tests them
    # two GPT-4o calls — more robust than single-pass generation
    llm = ChatOpenAI(model="gpt-4o", temperature=0.1, openai_api_key=state["api_key"])
    ...
    # Growth caps by maturity
    # growth caps by maturity — new stores can grow 40%, mature ones cap at 13%
    growth_caps = {"Mature": 0.13, "Mid": 0.20, "New": 0.40}

    # --- proposer: calculate target cascade ---
    ...
    for store, data in baseline_data.items():
        ...
        # Check for override in assumptions
        ...
        if override:
            ...
            # Achievement adjustment: underperformers get moderated targets
            ach_adj = (data["achievement_pct"] - 100) / 100 * 0.3
            growth_rate = min(cap, reg_growth + ach_adj + floor_change + fc_bonus)
            growth_rate = max(0.05, growth_rate)  # Minimum 5% growth
        ...
        for cat, cat_target in category_targets.items():
            ...
            for _, brand_row in cat_brands.iterrows():
                ...
                # Space multiplier from assumptions
    ...
    # --- proposer llm — format the target plan ---
...
**REGIONAL BREAKDOWN**
North / South / East / West targets and growth rates.

**STORE-BY-STORE TARGETS**
For each store: FY26 target, growth rate, key rationale (maturity, floor changes, regional index).

**CATEGORY HIGHLIGHTS**
Top 3 categories by absolute target growth.""")
    ])

    # --- challenger llm — autogen challenge pattern ---
...
# --- node 3 — loyalty planning agent ---

def loyalty_planning_node(state):
    # sets per-store loyalty penetration targets with stretch based on FY25 baseline
    llm = ChatOpenAI(model="gpt-4o", temperature=0.1, openai_api_key=state["api_key"])
    ...
    for _, row in store_loyalty.iterrows():
        ...
        # Set FY26 penetration target based on FY25 baseline
        # stretch factor depends on how far the store already is from target
        if fy25_pen > 0.50:
            stretch = 0.03   # Already strong — moderate stretch
        elif fy25_pen > 0.35:
            stretch = 0.06   # Growth zone
        else:
            stretch = 0.10   # Needs significant push

        fy26_pen_target = min(0.72, fy25_pen + stretch)

        # FY26 revenue target from cascade
        fy26_revenue = target_data.get(store, {}).get("fy26_target", row["FY25_Total_Revenue"] * 1.15)

        # Loyalty revenue target
        loyalty_rev_target = round(fy26_revenue * loyalty_contrib, 2)

        # New enrollment target
        current_members = int(row["FY25_Active_Members"])
        target_members = int(current_members * (1 + stretch * 2))
        enrollment_target = int((target_members - current_members) * 1.25)  # 25% churn buffer
...
# --- node 4 — business plan writer (word document) ---

def business_plan_writer_node(state):
    # synthesises all agents into narrative then generates Word document
    llm = ChatOpenAI(model="gpt-4o", temperature=0.1, openai_api_key=state["api_key"])
    ...
    # Generate narrative for the plan
    response = llm.invoke([
...
def generate_word_document(state, plan_text):
    """Generate a formatted Word document of the annual business plan."""
    try:
        ...
        # Brand OTB Summary
        doc.add_heading("BRAND OTB SUMMARY (TOP BRANDS)", level=2)
        ...
        if not otb_df.empty:
            ...
            ).reset_index().sort_values("OTB_Value", ascending=False).head(20)
...
def build_planning_workflow():
    ...
    workflow.add_node("target_cascade", target_cascade_node)
    workflow.add_node("loyalty_planning", loyalty_planning_node)
    workflow.add_node("business_plan_writer", business_plan_writer_node)

    workflow.set_entry_point("baseline_analysis")
    workflow.add_edge("baseline_analysis", "target_cascade")
    workflow.add_edge("target_cascade", "loyalty_planning")
    workflow.add_edge("loyalty_planning", "business_plan_writer")
    workflow.add_edge("business_plan_writer", END)

    memory = MemorySaver()
    return workflow.compile(
        checkpointer=memory,
        interrupt_before=["business_plan_writer"]  # Human reviews targets before final plan
    )


def get_initial_planning_state(
    company_growth_pct: float,
    regional_growth: dict,
    loyalty_target_pct: float,
    loyalty_revenue_contrib_pct: float,
    api_key: str,
) -> PlanningState:
